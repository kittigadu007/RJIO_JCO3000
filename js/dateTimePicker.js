...
(function($){		  
	...
	*/
    function Scroller(elm, settings){
        var that = this, e = elm, elm = $(e), theme, s = $.extend({}, defaults), m, dw, iv = {}, tv = {}, input = elm.is('input'), visible = false;
        // Private functions
        ...
        /*
         * Description: Adding ul for wheel and li for numbers dynamically.
         * Method: scrollToPos
         * params: time -
         * params: orig - selected number in a wheel
         * params: index - wheel Index. 0 for MM, 1 for DD ...
         * params: manual - returns true or false
         * params: dir - direction of wheel 1 for upward 2 for downward
         */
        function scrollToPos(time, orig, index, manual, dir){
            ...
            $('.dww ul', dw).each(function(i){
                var t = $(this), cell = $('li[data-val="' + that.temp[i] + '"]', t), x = cell.index(), v = scrollToValid(cell, x, i, dir), sc = i == index || index === undefined;
                if (x != v || sc) 
        ...
        /*
         * Description: function for get values from wheels
         * Method: scrollToValid
         * params: cell - li data
         * params: val - wheels value
         * params: i - wheels position
         * params: dir - direction of wheel 1 for upward 2 for downward
         */
        ...
        /**
         * checks the current using method
         */
        ...
        that.show = function(){
            ...
            var hi = s.height, thi = s.rows * hi;
            
            // Parse value from input
            read();
            
            // Create wheels containers
            var html = '<div class="' + s.theme + '">' + ('<div class="dw dwbg dwi"><div class="dwwr">');
            ...
            dw.delegate('.dwwl', 'DOMMouseScroll mousewheel', function(e){
                if (!s.readonly) {
                    e.preventDefault();
                    e = e.originalEvent;
                    var delta = e.wheelDelta ? (e.wheelDelta / 120) : (e.detail ? (-e.detail / 3) : 0), t = $('ul', this), p = +t.data('pos'), val = Math.round(p - delta);
                    setGlobals(t);
                ...
                if (!s.readonly) {
                    ...
                    e.preventDefault();
                    e.stopPropagation();
                    var t = $(this).closest('.dwwl').find('ul'), func = $(this).hasClass('dwwbp') ? plus : minus;
                    setGlobals(t);
                    clearInterval(timer);
    ...
    /*
     * calculate scroller values when selecting the dates
     */
    ...
    /*
     * genarating Datetime wheels
     * change the bellow properties for managing DateTime wheels
     */
    var scrollers = {}, timer, empty = function(){
    }, h, min, max, inst, // Current instance
 date = new Date(), uuid = date.getTime(), move = false, target = null, start, stop, startTime, endTime, pos, mod = document.createElement('modernizr').style, has3d = testProps(['perspectiveProperty', 'WebkitPerspective', 'MozPerspective', 'OPerspective', 'msPerspective']) && 'webkitPerspective' in document.documentElement.style, prefix = testPrefix(), touch = ('ontouchstart' in window), START_EVENT = touch ? 'touchstart' : 'mousedown', MOVE_EVENT = touch ? 'touchmove' : 'mousemove', END_EVENT = touch ? 'touchend' : 'mouseup', defaults = {
        // Options
        width: 24,
        height: 12,
        rows: 3,
        ...
        parseValue: function(val, inst){
            var w = inst.settings.wheels, val = val.split(' '), ret = [], j = 0;
    ...
    $(document).bind(END_EVENT, function(e){
        if (move) {
            ...
            var time = new Date() - startTime, val = pos + (start - stop) / h;
            val = val > (max + 1) ? (max + 1) : val;
            val = val < (min - 1) ? (min - 1) : val;
...
(function ($) {
	/*
	* Declaring Required values to genarate Date and time.
	*/		   
    var date = new Date(),
        defaults = {
            dateFormat: 'mm/dd/yy',
