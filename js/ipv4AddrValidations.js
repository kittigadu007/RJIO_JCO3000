 ...
 * Created on 1stNov Oct 2012 - Vivek
...
function ipSnetMaskOctetCheck(snetMaskObjArr){
    for (var i = 0; i < snetMaskObjArr.length; i++) {
        ...
        if (octet == "" || isNaN(octet)) {
            alert("Please enter valid value for octet " + (i + 1));
            return false;
        }
        if ((parseInt(octet, 10) < 0) || (parseInt(octet, 10) > 255)) {
            alert("Please enter valid value for octet " + (i + 1));
...
function ipSnetMaskFormatCheck(ipSnetMaskId){
    ...
    if (snetMaskObjArr.length != 4) {
        alert("Please enter valid number of octets.");
        return false;
    }
    /* Check IP Address Octet Range */
...
function ipRangeValidate(ipAddrStartId, ipAddrEndId){
    ...
    if (errorFlag) {
        var errorStr = "End IP address must be greater than or equal to the start IP address";
        alert(errorStr)
        endIpObj.focus();
        return false;
    }
...
function ipv4Validate(tblIPAddrId, ipv4AddrTypeStr, optFlag, alertFlag, prefixStr, suffixStr, octetShowFlag){
	...
	if (obj.value == "127.0.0.1") {
		alert("IP Address cannot have loop back address 127.0.0.1");
		obj.focus();
		return false;
	}
    if (!obj || obj.disabled) 
...
function subnetValidation(ipAddrId_check, ipAddrId_mask, ipAddrId, snetMaskId){
    ...
    objIpAddrId = document.getElementById(ipAddrId); // get IP object
    if (!objIpAddrId) 
        return;
    objArr = objIpAddrId.value.split(".");
    if (!objArr.length) 
        return false;
    ipAddrBits = getNetworkAddress(objArr); // get IP Address bits 
    objSnetMaskId = document.getElementById(snetMaskId); // get subnet mask object
    if (!objSnetMaskId) 
        return;
    /**/ 
     /*getting object of the ip and subnet mask of subnet which is being checked*/
    objIpAddr_chk = document.getElementById(ipAddrId_check); // get IP object
    objArrChk = objIpAddr_chk.value.split(".");
    if (!objArrChk.length) 
        return false;
    ipAddrBitsChk = getNetworkAddress(objArrChk); // get IP Address bits 
    ipAddrId_maskObj = document.getElementById(ipAddrId_mask); // get subnet mask object
    if (!ipAddrId_maskObj) 
        return;
    ...
    ipAddrId_checkLow = convert_addr(firstIPChk); // get decimal value of ip 
    ipAddrId_checkHigh = convert_addr(lastIPChk); // get decimal value of ip 
    lowIpVal = convert_addr(firstIP); // get decimal value of ip
    highIpVal = convert_addr(lastIP); // get decimal value of ip
...
function isDhcpInSubnet(ipAddrId, snetAddrId, dhcpAddrId){
    ...
    var ipAddrIdBits = getNetworkAddress(ipAddrIdObjArr); // get IP Address bits 
    ...
    var lowIpVal = convert_addr(lowIpAddr); // get decimal value of ip
    var highIpVal = convert_addr(highIpAddr); // get decimal value of ip 
...
/* 51350 starts */
/**
 * @method calcSubnetbits
 * @param subnetChars - subnet value
 */

function calcSubnetbits(subnetChars)
{
    sumofbits=0;
    var bytes = subnetChars.split('.');
    tmpvar = bytes[0];
    if (isNaN(tmpvar)){
        alert('Invalid');
        return -1;
...
/**
 * @method h_countbitsfromleft
 * @param num - count value
 */
