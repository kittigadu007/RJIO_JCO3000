...
function uploadImage (animationDelay, fileId, errMsg)
	...
	var proceed = confirm ('Preparing to upgrade firmware image. The upgrade process may take a few minutes. Are you sure you want to proceed?\n' +
	...
	if (proceed)
		...
		document.getElementById ('lblWarning').innerHTML = "WARNING: The upgrade process takes a few minutes.<br>" +
        	                   "Interrupting the upload or powering off the system during the upload may damage the firmware.<br>" +
...
function uploadImageandCheckmemory (animationDelay, fileId, errMsg)
	{
	if (accessLevelCheck () == false)
		return false;
	...
	if (proceed)
		{
      if (memoryCheck (animationDelay) == false)
        return false;
		}
	return false;
	}
...
function menuDisable ()
	...
	if(obj)
	    {
	    var anchorObjs = obj.getElementsByTagName ("a");
	    var len = anchorObjs.length;
	    for (i = 0; i < len; ++i)
	    anchorObjs [i].onclick = function () {
      alert("Firmware upgrade in progress…\nPlease try after completion of firmware upgrade.");
      return false;};
	    }
...
function uploadImageSizeCheck(){
	var uploadFileObj = document.getElementById('txtUploadFile');
	if (uploadFileObj && !uploadFileObj.disabled && uploadFileObj.value != '') {
		var fSize = uploadFileObj.files[0].size;
		if (fSize > 104857600)
		{
			alert("Invalid Firmware, Please upload a valid Firmware");
			return false;
		} 
	}
}
function check(objId,exten,flag)
    ...
    /* added changes for Firmware check during upgrade for model name starts */
    var uploadedFilename = $('#txtUploadFile').val().split('\\').pop();
    var modelNameobj = document.getElementById('tf1_hid_modelName');
    if(modelNameobj && uploadedFilename.indexOf(modelNameobj.value) == -1 ){
      alert("Please select firmware for "+modelNameobj.value);
      /* Only For IE Browsers */
      document.getElementById('txtUploadFile').parentNode.innerHTML = document.getElementById('txtUploadFile').parentNode.innerHTML;
     $('#txtUploadFile').val("");
      return false;
    }
    /* added changes for Firmware check during for model name ends */
    /* upload file check starts */
	if($.browser.msie)
    {
     	if (navigator.userAgent.indexOf("MSIE 9") != -1) { 
        var objFSO = new ActiveXObject("Scripting.FileSystemObject");
        var sPath = document.getElementById("txtUploadFile").value;
...
/* upload file check ends */
	...
	if (ext.length > 4)
		...
		if(flag == 1)
			...
			return uploadImage (50, 'txtUploadFile', 'Please enter the full path and file name of the image to be uploaded');
		else if(flag == 3)
			return restoreSettingsFromFile (10, 'RestoreFileObjects', 'Please enter the full path and file name containing settings to be restored');
  			...
  			return uploadImageandCheckmemory (50, 'txtUploadFile', 'Please enter the full path and file name of the image to be uploaded');
    else
			return uploadImage (50, 'txtUploadFile', 'Please enter the full path and file name of the image to be uploaded');
		}
	}
...
/*
	...
	if (ext.length > 4)
		...
		if(flag == 1)
			...
			return uploadImage (50, objId, 'Please select the file name of the image to be uploaded.');
        else
			return uploadImage (50, objId, 'Please select the file name of the image to be uploaded.');
//		}
	}
...
function memoryCheck(animationDelay) {

    var dataString = "button.freeMemCheck.freeMemCheck=check&thispage=freeMemCheck.html";
    $.ajax({
        url : "platform.cgi?page=freeMemCheck.html",
        type: "POST",  
        data : dataString,
        success: function(data, textStatus, jqXHR)
        {
            if($.trim(data) == "YES" ){       
              menuDisable ();
              document.getElementById ('lblWarning').innerHTML = "WARNING: The upgrade process takes a few minutes.<br>" +"Interrupting the upload or powering off the system during the upload may damage the firmware.<br>" +"Please wait until the upgrade process finishes before browsing any sites from your browser.";
              $(".midBg div.msgError").show();
              upgradeStarted = true;
              upgradeProgressShow (animationDelay);
              $("#tf1_frmUpgradeViaNetwork").submit();
            } else {
                alert("Sorry system is busy. please try again after few minutes..");     
                return false;
            }
        },
